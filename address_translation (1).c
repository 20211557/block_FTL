...
static LOGICAL_BLOCK_MAP logicalBlockMapPtr;
...
void InitSliceMap()
{
	...
	for(blockAddr=0; blockAddr<USER_BLOCKS_PER_SSD ; blockAddr++){
		logicalBlockMapPtr.logicalBlock[blockAddr].virtualBlockAddr = VBA_NONE;
	}
...
{
	/*
	unsigned int virtualSliceAddr, logicalBlockAddr, virtualBlockAddr, offset, dieNo;
	unsigned int currentBlock;

	if(logicalSliceAddr < SLICES_PER_SSD)
	{
		virtualSliceAddr = logicalSliceMapPtr->logicalSlice[logicalSliceAddr].virtualSliceAddr; // LBA -> VBA
	...
	*/
	unsigned int virtualSliceAddr, logicalBlockAddr, virtualBlockAddr, dieNo, blockNo, pageNo;

	if(logicalSliceAddr < SLICES_PER_SSD)
	{
		logicalBlockAddr = logicalSliceAddr / SLICES_PER_BLOCK;
		virtualBlockAddr = logicalBlockMapPtr.logicalBlock[logicalBlockAddr].virtualBlockAddr;
		if(virtualBlockAddr == VBA_NONE){
			//xil_printf("read empty page\r\n");
			return VBA_FAIL;
		}
		else{
			// 매핑 테이블에 저장된 주소로부터 Die 번호와 Block 번호를 추출
			dieNo = Vsa2VdieTranslation(virtualBlockAddr);
			blockNo = Vsa2VblockTranslation(virtualBlockAddr);
			unsigned int totalPages = virtualBlockMapPtr->block[dieNo][blockNo].currentPage;
			if(totalPages == 0){
				assert(!"[WARNING] There is no pages in the block [ WARNING]");
			}
			pageNo = (logicalSliceAddr % SLICES_PER_BLOCK) % totalPages;
...
unsigned int AddrTransWrite(unsigned int logicalSliceAddr)
{
	...
	*/
	...
	{
		logicalBlockAddr = logicalSliceAddr / SLICES_PER_BLOCK;
		pageNo = logicalSliceAddr % SLICES_PER_BLOCK;
		virtualBlockAddr = logicalBlockMapPtr.logicalBlock[logicalBlockAddr].virtualBlockAddr;
		if(virtualBlockAddr == VBA_NONE)
		{
			...
			{
				// Block Mapping이므로 블록의 시작 VSA(page 0)를 대표 주소로 테이블에 기록
				virtualBlockAddr = Vorg2VsaTranslation(dieNo, blockNo, 0);
				logicalBlockMapPtr.logicalBlock[logicalBlockAddr].virtualBlockAddr = virtualBlockAddr;
				virtualBlockMapPtr->block[dieNo][blockNo].free = 0;
				virtualBlockMapPtr->block[dieNo][blockNo].currentPage = 1;
		...
		else
		{
			dieNo = Vsa2VdieTranslation(virtualBlockAddr);
			blockNo = Vsa2VblockTranslation(virtualBlockAddr);
			pageNo = virtualBlockMapPtr->block[dieNo][blockNo].currentPage++;
			if(pageNo >= USER_PAGES_PER_BLOCK)
